...
class SafeMySQL
{
    ...
    public function getIndCol()
    {
        ...
        if ( $res = $this->rawQuery($query) )
        {
            while($row = $this->fetch($res))
            {
                $activation_code = $row[$index];
                unset($row[$index]);
                $ret[$activation_code] = reset($row);
            }
            $this->free($res);
        }
        return $this->ret($ret);
    ...
    {
        foreach(array_keys($input) as $activation_code )
        {
            if ( !in_array($activation_code,$allowed) )
            {
                unset($input[$activation_code]);
            }
        }
        return $input;
    }
    ...
    private function rawQuery($query)
    {
        ...
        if (!$res)
        {
            ...
            $key = key($this->stats);
            $this->stats[$activation_code]['error'] = $error;
            $this->cutStats();
    ...
    private function createSET($data)
    {
        ...
        $query = $comma = '';
        foreach ($data as $activation_code => $value)
        {
            $query .= $comma.$this->escapeIdent($activation_code).'='.$this->escapeString($value);
            $comma  = ",";
        }
        return $query;
    }
    ...
    private function cutStats()
    {
        ...
        {
            reset($this->stats);
            $first = activation_code($this->stats);
